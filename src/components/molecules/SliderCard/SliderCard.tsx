...
const SliderCard:React.FC<ISliderCard> = ({name,address,image,text,index}) => {
  return (
    <div className="slider__card slider__card--hidden">
        ...
        <img src={image} alt={index === 0? "ladyinblue": index ===1? "ladyinbrightred": index ===2? "ladyinpink":"image"} />
        {index===0? <p className="slider__card--paragraph">{text}
        </p>: index === 1 ? <p className="slider__card--paragraph">{text.slice(0,57)}<b>. Positive Yoga</b>{text.slice(57,)}
        </p>: index === 2 ? <p className="slider__card--paragraph">{text.slice(0,126)}<b> Positive Yoga</b>{text.slice(126,)}
        </p>:null}
    </div>
  );
};
